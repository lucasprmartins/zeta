...
export function parseMarkdown(markdown: string): GuideBlock[] {
  ...
  for (const line of markdown.replace(/\r\n?/g, "\n").split("\n")) {
    ...
    if (
      /^(?:#{4,}\s|```|~~~|>\s|[-*+]\s|\d+[.)]\s)/.test(line) ||
      /(?<!\\)!\[[^\]]*\]\(/.test(line)
    ) {
      throw new Error(
        "Use somente texto e títulos H1, H2 e H3. Imagens, listas e outros blocos ainda não são suportados."
      );
    }
    ...
    if (heading) {
      ...
      blocks.push({
        ...
        text: unescapeText(heading[2]!),
      });
    } else {
      paragraph.push(unescapeText(line));
    }
  }
  flush();
  if (blocks.length > 1000) {
    throw new Error("Use até 1000 blocos por guia.");
  }
  return blocks;
}
...
export function parseGuideFile(source: string) {
  ...
  if (!match) {
    throw new Error(
      "O arquivo precisa de metadados entre linhas --- no início."
    );
  }
  ...
  for (const line of match[1]!.split("\n")) {
    ...
    if (
      ...
      throw new Error("Metadado inválido ou duplicado.");
    }
    const value: unknown = field[2]!.startsWith('"')
      ? JSON.parse(field[2]!)
      : field[2]!;
    if (typeof value !== "string") {
      throw new Error("Metadados textuais inválidos.");
    }
