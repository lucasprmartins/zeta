...
`/admin/console` permite configurar separadamente `allowSignUp` e `requireApproval`. Ambas são persistidas em `console.registration`. O padrão é cadastro aberto, sem aprovação; fechar o cadastro não apaga a preferência de aprovação.
