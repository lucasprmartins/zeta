...
- `AppShell` contém sidebar, cabeçalho de 64 px e área principal; `AppSidebar` centraliza navegação e acesso ao perfil pelo bloco da conta. A trilha do cabeçalho vem de `pageBreadcrumb`, que nomeia seção e página com os rótulos da própria navegação — rotas `/admin/*` ficam sob Administração, `/help/guides*` sob Ajuda e as demais sob Workspace. Abaixo de 640 px só a página aparece.
