...
export async function renameProject(root: string, name: string) {
  if (!validName(name)) {
    throw new Error(
      "Use de 1 a 50 caracteres: letras minúsculas, números e hífens, começando com letra."
    );
  }
  ...
  if (!validName(previous)) {
    throw new Error("O nome atual exige renomeação manual.");
  }
  const title = (value: string) => value[0]!.toUpperCase() + value.slice(1);
  // Lista explícita: nunca percorre .git, .env, migrations ou arquivos do domínio.
  const changes = await Promise.all(
    identityFiles.map(async (path) => {
      ...
      return {
        ...
        content: original
          .replace(
            new RegExp(`(?<![a-zA-Z0-9_-])${previous}(?![a-zA-Z0-9_-])`, "g"),
            name
          )
          .replace(
            new RegExp(
              `(?<![a-zA-Z0-9_-])${title(previous)}(?![a-zA-Z0-9_-])`,
              "g"
            ),
            title(name)
          ),
  ...
  for (const change of [...changes.slice(1), changes[0]!]) {
...
export function assertLocalDatabase(env: Record<string, string | undefined>) {
  ...
  if (env.DATABASE_URL !== "postgresql://app:app@localhost:5432/app") {
    throw new Error(
      "DATABASE_URL não corresponde ao Compose local. Use --database skip e configure/aplique migrations manualmente."
    );
  }
}
