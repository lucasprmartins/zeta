...
beforeAll(async () => {
  ...
  handler = await createClientHandler({
    directory,
    apiOrigin: upstream.url.origin,
...
test("serves SPA navigation, static assets and HEAD with the appropriate cache policy", async () => {
  const page = await handler(
    request("/tasks?status=pending", { headers: { accept: "text/html" } })
  );
...
test("forwards body, query, cookies and origin and preserves multiple Set-Cookie headers", async () => {
  ...
  expect(received).toEqual({
    path: "/api/auth/sign-in/email?test=1",
    body: '{"email":"qa@example.com"}',
    cookie: "session=old",
    origin: "https://app.example.com",
...
test("forwards all API prefixes without treating similar page names as API routes", async () => {
  ...
  expect(await (await handler(request("/_health"))).json()).toEqual({
    status: "ok",
  });
});
test("preserves redirects and compressed bodies from the API", async () => {
  ...
  expect(
    new TextDecoder().decode(Bun.gunzipSync(await compressed.arrayBuffer()))
  ).toBe("compressed response");
});
test("fails at startup when the frontend build is missing", async () => {
  ...
  try {
    ...
    ).rejects.toThrow("Build do cliente");
  } finally {
...
test("returns 502 when the API is unavailable", async () => {
  ...
  const unavailable = await createClientHandler({
    directory,
    apiOrigin,
    publicOrigin: "https://app.example.com",
  });
