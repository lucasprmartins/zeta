...
export async function createClientHandler(options: { directory: string; apiOrigin: string; publicOrigin: string }) {
  const apiOrigin = new URL(options.apiOrigin);
  const publicOrigin = new URL(options.publicOrigin);
  if (!["http:", "https:"].includes(apiOrigin.protocol) || !["http:", "https:"].includes(publicOrigin.protocol)) {
  ...
  for await (const path of new Bun.Glob("**/*").scan({ cwd: options.directory, onlyFiles: true })) {
    if (path.split("/").some((part) => part.startsWith("."))) continue;
    files.set(`/${path}`, Bun.file(join(options.directory, path)));
  }
  const index = files.get("/index.html");
  if (!index) throw new Error("Build do cliente não encontrado. Execute bun run build.");

  return async (request: Request): Promise<Response> => {
    const url = new URL(request.url);
    if (url.pathname === "/_health" && (request.method === "GET" || request.method === "HEAD")) {
      return new Response(request.method === "HEAD" ? null : '{"status":"ok"}', { headers: { "content-type": "application/json" } });
    }
    if (apiPaths.some((path) => url.pathname === path || url.pathname.startsWith(`${path}/`))) {
      ...
      try {
        ...
        return Response.json({ message: "API indisponível." }, { status: 502, headers: { "cache-control": "no-store" } });
      }
    }
    ...
    if (!pathname.split("/").some((part) => part.includes(".")) && request.headers.get("accept")?.includes("text/html")) {
      return new Response(request.method === "HEAD" ? null : index, { headers: { "content-type": "text/html; charset=utf-8", "cache-control": "no-cache" } });
    }
