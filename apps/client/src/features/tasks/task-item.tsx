...
export function TaskItem({ task, pending, onStatus, onDelete, onEdit }: {
  ...
  return <li className="group grid grid-cols-[44px_minmax(0,1fr)] items-start gap-2 border-b px-3 py-3 last:border-b-0 hover:bg-sidebar/70 sm:flex sm:items-center sm:gap-3 sm:px-4">
    <button type="button" role="checkbox" aria-checked={completed} aria-label={`${completed ? "Reabrir" : "Concluir"} ${task.title}`} disabled={pending || !can(permissions.tasks.setStatus)} onClick={() => onStatus(task)}
    ...
    <button type="button" onClick={() => onEdit(task)} disabled={pending || !can(permissions.tasks.update)} className="min-h-11 min-w-0 flex-1 rounded-sm py-1 sm:min-h-0 text-left focus-visible:outline-2 focus-visible:outline-offset-4" aria-label={`Abrir ${task.title}`}>
    ...
    <div className="col-start-2 flex shrink-0 justify-end gap-1 sm:gap-0">
      <Can permission={permissions.tasks.update}><Button variant="ghost" size="icon" className="w-auto gap-2 px-3 text-muted-foreground sm:size-9 sm:px-0" aria-label={`Editar ${task.title}`} title="Editar tarefa" disabled={pending} onClick={() => onEdit(task)}><PencilSimpleIcon /><span className="sm:hidden">Editar</span></Button></Can>
      <Can permission={permissions.tasks.delete}><Button variant="ghost" size="icon" className="w-auto gap-2 px-3 text-muted-foreground sm:size-9 sm:px-0" aria-label={`Excluir ${task.title}`} title="Excluir tarefa" disabled={pending} onClick={() => onDelete(task)}><TrashIcon /><span className="sm:hidden">Excluir</span></Button></Can>
    </div>
  </li>;
}
