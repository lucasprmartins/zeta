...
export const registrationPolicyQuery = queryOptions({ queryKey: ["registration-policy"], queryFn: ({ signal }) => rpc.access.registrationPolicy(undefined, { signal }), staleTime: 0, refetchInterval: 15000 });
export const registrationStatusQuery = (userId: string) => queryOptions({ queryKey: [...accessKeys.all(userId), "registration"], queryFn: ({ signal }) => rpc.access.registrationStatus(undefined, { signal }), refetchInterval: 15000 });
export const approvalsQuery = (userId: string) => infiniteQueryOptions({
  queryKey: [...accessKeys.all(userId), "approvals", "infinite"], initialPageParam: 1,
  queryFn: ({ pageParam, signal }) => rpc.access.pendingUsers({ page: pageParam }, { signal }),
  getNextPageParam: (page, _, previous) => page.hasMore ? previous + 1 : undefined,
});
