  ...
  return <PageContent>
    ...
    {error ? <ErrorNotice message="Não foi possível carregar o resumo." retry={() => { void pending.refetch(); void completed.refetch(); }} /> : loading || !pending.data || !completed.data ? <Loading label="Carregando seu resumo…" /> : <>
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        ] as const).map(({ label, value, icon: Icon, status }) => <Link key={status} to="/tasks" search={{ status }} className="group rounded-xl border p-5 transition-colors hover:bg-sidebar">
      ...
      <div className="space-y-6">
        ...
        <section className="overflow-hidden rounded-xl border" aria-labelledby="recent-tasks">
          ...
          {pending.data.items.length === 0 ? <div className="px-6 py-12 text-center"><CheckSquareIcon className="mx-auto mb-4 size-6 text-muted-foreground" /><p className="text-sm font-medium">{total === 0 ? "Pronto para começar?" : "Tudo concluído por aqui."}</p><p className="mt-2 text-xs text-muted-foreground">{total === 0 ? "Acesse Tarefas e registre seu primeiro passo." : "Suas próximas tarefas aparecerão neste espaço."}</p></div> :
