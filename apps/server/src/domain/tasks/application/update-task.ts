...
export function updateTask(tasks: TaskRepository, now: () => string) {
  return async (input: { id: string; ownerId: string; title: string; description: string }) => {
    const task = await tasks.findById(input.id, input.ownerId);
    if (!task) throw new TaskNotFoundError();
    const updated = task.edit({ title: input.title, description: input.description }, now());
