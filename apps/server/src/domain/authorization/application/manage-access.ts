...
export function manageAccess(repository: AccessRepository, available: readonly string[], generateId: () => string) {
  ...
  return {
    ...
    save(actorId: string, input: { id?: string; name: string; grants: string[]; color?: string }) {
      const fields = roleFields(input.name, input.id === "admin" ? [] : input.grants, available, input.color);
      return repository.transaction(async (store) => {
        await authorize(store, actorId);
        const existing = input.id ? await store.role(input.id) : null;
        if (input.id && !existing) throw new AccessError("NOT_FOUND", "Papel não encontrado.");
        const roles = await store.roles();
        if (roles.some((role) => role.id !== input.id && role.name.toLowerCase() === fields.name.toLowerCase())) throw new AccessError("CONFLICT", "Já existe um papel com esse nome.");
        if (!existing && roles.length >= 100) throw new AccessError("CONFLICT", "Limite de 100 papéis atingido.");
        const role = { id: existing?.id ?? generateId(), ...fields, color: input.color === undefined ? existing?.color ?? fields.color : fields.color, grants: existing?.id === "admin" ? effectiveRoleGrants(existing, available) : fields.grants, protected: existing?.protected ?? false };
    ...
    remove(actorId: string, id: string) {
      return repository.transaction(async (store) => {
        ...
        if (!role) throw new AccessError("NOT_FOUND", "Papel não encontrado.");
        if (role.protected) throw new AccessError("FORBIDDEN", "Papéis do sistema não podem ser excluídos.");
        if (await store.assigned(id)) throw new AccessError("CONFLICT", "Reatribua os usuários deste papel antes de excluí-lo.");
...
export async function assignRole(store: AccessStore, userId: string, roleId: string) {
  const [account, role] = await Promise.all([store.user(userId), store.role(roleId)]);
  if (!account || !role) throw new AccessError("NOT_FOUND", "Conta ou papel não encontrado.");
  if (account.role === "admin" && !account.banned && roleId !== "admin" && await store.activeAdmins() <= 1) throw new AccessError("CONFLICT", "Mantenha pelo menos um administrador ativo.");
