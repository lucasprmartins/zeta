...
export function manageGuides(
  ...
  async function get(slug: string) {
    ...
    if (!guide) {
      throw new GuideError("NOT_FOUND", "Guia não encontrado.");
    }
    return guide;
  }
  return {
    ...
    async list(page: number, grants: readonly string[]) {
      ...
      return {
        ...
        items: result.items
          .filter((guide) => readable(guide, grants))
          .map((guide) => ({
            slug: guide.slug,
            title: guide.published!.title,
            section: guide.published!.section,
            order: guide.published!.order,
          })),
      };
    },
    async save(input: {
      ...
      const slug = guideSlug(input.slug),
        draft = fields(input.draft);
      ...
      if (
        ...
        throw new GuideError(
          "CONFLICT",
          "Este guia foi alterado ou já existe. Recarregue antes de salvar."
        );
      }
      if (input.action === "publish" && !draft.markdown.trim()) {
        throw new GuideError(
          "BAD_REQUEST",
      ...
      const saved = previous
        ? await repository.replace(guide, previous.version)
        : await repository.insert(guide);
      if (!saved) {
        throw new GuideError(
          "CONFLICT",
          "Este guia foi alterado por outra pessoa. Recarregue antes de salvar."
        );
      }
      return guide;
    },
