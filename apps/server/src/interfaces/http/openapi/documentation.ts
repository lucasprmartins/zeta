...
export async function createDocumentation(router: AppRouter, authentication: Authentication) {
  const [tasks, auth] = await Promise.all([
    new OpenAPIGenerator({ schemaConverters: [schemaConverter] }).generate(router),
  ...
  return openapi({
    ...
    documentation: {
      info: {
        title: "Zeta API",
        version: "0.1.0",
        description: "Cadastre-se ou entre com email ou nome de usuário e senha nos endpoints de autenticação. O cookie de sessão será usado nas chamadas de tarefas feitas nesta mesma origem. Os endpoints REST usam as mesmas procedures e regras do cliente oRPC.",
      },
      ...
      components: {
        ...
        securitySchemes: {
          ...
          sessionCookie: { type: "apiKey", in: "cookie", name: "better-auth.session_token", description: "Faça login pelo endpoint sign-in/email. O navegador envia o cookie automaticamente (prefixo __Secure- em HTTPS)." },
        },
      },
    },
  });
