...
const sections = [
  {
    name: "Acesso",
    description: "Comece aqui: crie sua conta, entre ou encerre a sessão.",
    endpoints: {
  ...
  },
  {
    name: "Sessões",
    description:
      "Consulte a sessão atual e gerencie os dispositivos conectados.",
    endpoints: {
  ...
  {
    name: "Conta",
    description: "Gerencie os dados do usuário e sua conta.",
    endpoints: {
  ...
  },
  {
    name: "Senhas",
    description:
      "Altere ou recupere sua senha. A recuperação exige um provedor de email configurado.",
    endpoints: {
  ...
  },
  {
    ...
    description:
      "Confirme o endereço de email. O envio exige um provedor de email configurado.",
    endpoints: {
  ...
  },
  {
    name: "Provedores",
    description:
      "Integrações com provedores externos. Configure o provedor antes de usar estas operações.",
    endpoints: {
  ...
  },
  {
    name: "Administração",
    description:
      "Gerenciamento de contas e sessões conforme as permissões do papel.",
    endpoints: {
  ...
  {
    name: "Diagnóstico",
    description: "Respostas de estado e erro do serviço de autenticação.",
    endpoints: {
...
export function organizeAuthSections(document?: OpenAPI.Document) {
  ...
  const orderedPaths = new Set([
    ...sections.flatMap((section) => Object.keys(section.endpoints)),
  ...
  for (const path of orderedPaths) {
    ...
    const summary =
      section && (section.endpoints as Record<string, string>)[path];
    const authPath = { ...item };
    for (const method of [
      "get",
      "post",
  ...
  const tags = sections
    .filter((section) => usedTags.has(section.name))
    .map(({ name, description }) => ({ name, description }));
  if (usedTags.has("Outras operações")) {
    tags.push({
      name: "Outras operações",
      description: "Endpoints adicionais de autenticação.",
    });
  }
  return { paths, tags };
}
