...
export type UserFields = { name: string; username: string; email: string; roleId: string; password?: string };
export interface UserManagement {
  create(headers: Headers, input: UserFields): Promise<AccessUser>;
  update(headers: Headers, input: UserFields & { userId: string }): Promise<AccessUser>;
}
