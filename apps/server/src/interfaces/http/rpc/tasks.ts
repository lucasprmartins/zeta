...
const procedure = protectedProcedure.errors({ BAD_REQUEST: {}, UNAUTHORIZED: {}, NOT_FOUND: {} }).use(async ({ next }) => {
...
export function createTasksRouter(useCases: TaskUseCases) {
  return {
    create: procedure.route({ ...route, method: "POST", path: "/tasks", summary: "Criar tarefa" }).input(createInput).output(taskOutput)
      .handler(({ input, context }) => useCases.create({ ...input, ownerId: context.user.id })),
    list: procedure.route({ ...route, method: "GET", path: "/tasks", summary: "Listar minhas tarefas" }).input(listInput).output(taskListOutput)
      .handler(({ input, context }) => useCases.list({ ...input, ownerId: context.user.id })),
    update: procedure.route({ ...route, method: "PATCH", path: "/tasks/{id}", summary: "Editar tarefa" }).input(updateInput).output(taskOutput)
      .handler(({ input, context }) => useCases.update({ ...input, ownerId: context.user.id })),
    setStatus: procedure.route({ ...route, method: "PATCH", path: "/tasks/{id}/status", summary: "Concluir ou reabrir tarefa" }).input(statusInput).output(taskOutput)
      .handler(({ input, context }) => useCases.setStatus({ ...input, ownerId: context.user.id })),
    delete: procedure.route({ ...route, method: "DELETE", path: "/tasks/{id}", summary: "Excluir tarefa" }).input(idInput).output(deleteOutput)
