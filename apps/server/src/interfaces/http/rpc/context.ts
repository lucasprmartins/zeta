...
export const requirePermission = (permission: Permission) => os.$context<{ user: { id: string; role: string | null; grants: string[] } }>().middleware(async ({ context, next }) => {
  if (!can(context.user.grants, permission)) throw new ORPCError("FORBIDDEN", { message: "Você não tem permissão para esta ação." });
  return next();
});
