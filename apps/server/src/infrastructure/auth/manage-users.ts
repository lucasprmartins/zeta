...
import type {
  UserFields,
  UserManagement,
} from "@server/interfaces/http/user-management";
...
function validate(input: UserFields, creating: boolean) {
  ...
  if (!name || name.length > 120) {
    throw new AccessError("BAD_REQUEST", "Use um nome de 1 a 120 caracteres.");
  }
  if (!/^[a-zA-Z0-9_.]{3,30}$/.test(username)) {
    throw new AccessError(
      "BAD_REQUEST",
      "O nome de usuário deve ter de 3 a 30 caracteres: letras sem acento, números, ponto ou sublinhado."
    );
  }
  if (email.length > 254 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    throw new AccessError("BAD_REQUEST", "Informe um e-mail válido.");
  }
  if (
    ...
    throw new AccessError(
      "BAD_REQUEST",
      "A senha deve ter de 8 a 128 caracteres."
    );
  }
...
function translate(error: unknown): never {
  ...
  if (error instanceof APIError) {
    ...
    if (
      typeof code === "string" &&
      /USERNAME.*TAKEN|USER_ALREADY_EXISTS|EMAIL_ALREADY/.test(code)
    ) {
      throw new AccessError(
        "CONFLICT",
    ...
    if (error.statusCode === 404) {
      throw new AccessError("NOT_FOUND", "Usuário não encontrado.");
    }
    if (error.statusCode < 500) {
      throw new AccessError(
        "BAD_REQUEST",
        "Confira os dados da conta e os requisitos da senha."
      );
    }
  }
...
export function createUserManagement(db: Database, env: Env): UserManagement {
  async function save(headers: Headers, raw: UserFields, targetId?: string) {
    ...
    try {
      return await db.transaction(async (tx) => {
        ...
        if (!(await store.role(input.roleId))) {
        ...
        if (targetId === undefined) {
          ...
          if (input.password !== undefined) {
            await auth.api.setUserPassword({
              headers,
              body: { userId: targetId, newPassword: input.password },
            });
  ...
  return {
    create: (headers, input) => save(headers, input),
    update: (headers, input) => save(headers, input, input.userId),
  };
}
