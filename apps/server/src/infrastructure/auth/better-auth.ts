...
import { defaultRegistrationPolicy, type RegistrationPolicy } from "@server/domain/authorization/entities/registration-policy";
...
export function createBetterAuth(db: DatabaseConnection, env: Env, policy: RegistrationPolicy = defaultRegistrationPolicy) {
  return betterAuth({
    ...
    emailAndPassword: { enabled: true, disableSignUp: !policy.allowSignUp, autoSignIn: !policy.requireApproval },
    user: { additionalFields: { approvalPending: { type: "boolean", required: false, defaultValue: false, input: false } } },
    databaseHooks: {
      user: { create: { before: async (account, context) => {
        const administrative = context?.path === "/admin/create-user";
        if (!administrative && !policy.allowSignUp) throw new APIError("FORBIDDEN", { code: "SIGNUP_DISABLED", message: "O cadastro de novas contas está desativado." });
        return { data: { ...account, approvalPending: !administrative && policy.requireApproval } };
      } } },
      session: { create: { before: async (session) => {
        const account = (await db.select({ pending: schema.user.approvalPending }).from(schema.user).where(eq(schema.user.id, session.userId)).limit(1))[0];
        if (account?.pending) throw new APIError("FORBIDDEN", { code: "ACCOUNT_PENDING_APPROVAL", message: "Sua conta aguarda aprovação de um administrador." });
      } } },
    },
...
export function createAuthentication(db: Database, env: Env, resolveGrants: (role: string) => Promise<string[]>): Authentication {
  ...
  return {
    ...
    async resolveUser(headers) {
      ...
      return session && !session.user.banned && !session.user.approvalPending ? { id: session.user.id, role: session.user.role ?? null, grants: await resolveGrants(session.user.role ?? "") } : null;
    },
  };
}
