...
function store(db: Pick<Database, "select" | "insert" | "update" | "delete">): AccessStore {
  ...
  return {
    async registrationPolicy() {
      const row = (await db.select().from(registrationSettings).where(eq(registrationSettings.id, "registration")))[0];
      return row ? { allowSignUp: row.allowSignUp, requireApproval: row.requireApproval } : { ...defaultRegistrationPolicy };
    },
    async saveRegistrationPolicy(policy) { await db.insert(registrationSettings).values({ id: "registration", ...policy }).onConflictDoUpdate({ target: registrationSettings.id, set: policy }); },
    async pendingUsers(page) {
      const items = await db.select(columns).from(user).where(eq(user.approvalPending, true)).orderBy(asc(user.createdAt), asc(user.id)).limit(21).offset((page - 1) * 20);
      return { items: items.slice(0, 20), hasMore: items.length > 20 };
    },
    async pendingCount() { return (await db.select({ total: count() }).from(user).where(eq(user.approvalPending, true)))[0]!.total; },
    async approve(userId) { return (await db.update(user).set({ approvalPending: false }).where(and(eq(user.id, userId), eq(user.approvalPending, true))).returning({ id: user.id })).length > 0; },
    ...
    async users(page, search) {
      ...
      const items = await db.select(columns).from(user).leftJoin(accessRoles, eq(user.role, accessRoles.id)).where(and(eq(user.approvalPending, false), search ? condition : undefined)).orderBy(asc(sql`coalesce(${accessRoles.name}, ${user.role})`), asc(user.role), asc(user.name), asc(user.id)).limit(21).offset((page - 1) * 20);
      return { items: items.slice(0, 20), hasMore: items.length > 20 };
    },
    ...
    async activeAdmins() { return (await db.select({ total: count() }).from(user).where(and(eq(user.role, "admin"), eq(user.banned, false), eq(user.approvalPending, false))))[0]!.total; },
  };
}
