...
function store(db: Pick<Database, "select" | "insert" | "update" | "delete">): AccessStore {
  const columns = { id: user.id, name: user.name, email: user.email, role: user.role, banned: user.banned };
  return {
    ...
    async users(page, search) {
      ...
      const items = await db.select(columns).from(user).leftJoin(accessRoles, eq(user.role, accessRoles.id)).where(search ? or(ilike(user.name, pattern), ilike(user.email, pattern)) : undefined).orderBy(asc(sql`coalesce(${accessRoles.name}, ${user.role})`), asc(user.role), asc(user.name), asc(user.id)).limit(21).offset((page - 1) * 20);
...
export function createAccessRepository(db: Database): AccessRepository {
