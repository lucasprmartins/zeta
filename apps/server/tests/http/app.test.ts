...
async function setup(options: { databaseDown?: boolean; failSave?: boolean; role?: string } = {}) {
  ...
  const rpc = (procedure: string, input?: unknown, user?: string) => app.handle(new Request(`http://localhost/rpc/tasks/${procedure}`, {
    method: "POST",
...
describe("HTTP e RPC", () => {
  test("health não depende do banco; readiness sinaliza indisponibilidade", async () => {
    ...
    expect((await app.handle(new Request("http://localhost/ready"))).status).toBe(503);
  });

  test("encaminha o corpo bruto ao handler de autenticação", async () => {
    const { app } = await setup();
    const response = await app.handle(new Request("http://localhost/api/auth/sign-in/email", {
      method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email: "test@example.com" }),
    }));
    expect(await response.json()).toEqual({ body: { email: "test@example.com" } });
  });
  ...
  test("valida entradas em runtime e regras do domínio", async () => {
    const { rpc, tasks } = await setup();
    for (const input of [null, {}, { title: 12 }, { title: " " }, { title: "x".repeat(121) }]) {
  ...
  test("usa o proprietário da sessão e impede acesso entre usuários", async () => {
    const { rpc, tasks } = await setup();
    const response = await rpc("create", { title: " API ", ownerId: "user-2" }, "user-1");
    expect(response.status).toBe(200);
    expect(tasks.items[0]?.toJSON()).toMatchObject({ title: "API", ownerId: "user-1" });
    expect(await (await rpc("list", undefined, "user-2")).json()).toEqual({ json: { items: [], total: 0, page: 1, pageSize: 20 } });
  ...
  test("publica Scalar e schemas concretos dos endpoints HTTP", async () => {
    ...
    expect(spec.paths["/api/tasks"].post.requestBody.content["application/json"].schema.required).toEqual(["title"]);
    expect(spec.paths["/api/tasks"].get.responses["200"].content["application/json"].schema.type).toBe("object");
    expect(spec.paths["/api/tasks"].get.security).toEqual([{ sessionCookie: [] }]);
  ...
  test("REST compartilha validação, autenticação e aplicação com RPC", async () => {
    const { app, rpc } = await setup();
    const rest = (method: string, body?: unknown, user?: string) => app.handle(new Request("http://localhost/api/tasks", {
      method,
      headers: { "content-type": "application/json", ...(user ? { "x-test-user": user } : {}) },
    ...
    const response = await rest("POST", { title: "REST", ownerId: "user-2" }, "user-1");
    expect(response.status).toBe(200);
    expect(await response.json()).toMatchObject({ title: "REST", ownerId: "user-1" });
    expect(await (await rest("GET", undefined, "user-2")).json()).toEqual({ items: [], total: 0, page: 1, pageSize: 20 });
