  ...
  const service = manageGuides(
    {
      ...
      insert: async (guide) => {
        if (records.has(guide.slug)) {
          return false;
        }
        records.set(guide.slug, guide);
        return true;
      },
      replace: async (guide, version) => {
        if (records.get(guide.slug)?.version !== version) {
          return false;
        }
        records.set(guide.slug, guide);
        return true;
...
test("rascunho não altera publicação e edição concorrente não sobrescreve", async () => {
  ...
  await service.save({
    slug: "inicio",
    draft: { ...fields, markdown: "Novo rascunho" },
  ...
  expect(
    await service.importMissing("inicio", {
      ...fields,
      title: "Arquivo antigo",
    })
  ).toBe(false);
  expect((await service.adminGet("inicio")).draft.markdown).toBe(
    "Novo rascunho"
  );
  await service.save({
    slug: "inicio",
...
test("Markdown preserva blocos básicos e textos literais sem executar HTML", () => {
  const blocks = [
    { type: "heading" as const, level: 1 as const, text: "Título" },
    { type: "paragraph" as const, text: "# texto literal" },
    { type: "paragraph" as const, text: "<script>alert(1)</script>" },
    { type: "paragraph" as const, text: "![exemplo](arquivo)" },
  ];
  blocks.push(
    { type: "paragraph", text: "\\![literal](arquivo)" },
    { type: "heading", level: 1, text: "![literal](arquivo)" }
  );
  expect(parseMarkdown(toMarkdown(blocks))).toEqual(blocks);
  expect(() => parseMarkdown("![imagem](https://example.com/img.png)")).toThrow(
    "Imagens"
  );
