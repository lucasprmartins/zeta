...
import {
  ...
  effectiveRoleGrants,
  roleFields,
} from "@server/domain/authorization/entities/role";
...
function setup() {
  ...
  const repository: AccessRepository = {
    ...
    assigned: async (id) =>
      [...users.values()].some((user) => user.role === id),
...
test("papéis normalizam nomes e concessões, rejeitando ações fora do catálogo", () => {
  expect(
    roleFields("  Operador  ", ["tasks:read", "tasks:read"], ["tasks:read"])
  ).toEqual({ name: "Operador", color: "#737373", grants: ["tasks:read"] });
  expect(() => roleFields("Cor inválida", [], [], "red")).toThrow(
    "hexadecimal"
  );
  for (const name of [" ", "x".repeat(61)]) {
    expect(() => roleFields(name, [], [])).toThrow();
  }
  expect(() =>
    roleFields("Operador", ["access:manage"], ["tasks:read"])
  ).toThrow("reservada");
});
...
test("aplicação revalida administradores e protege papéis do sistema e atribuições", async () => {
  ...
  await expect(
    service.save("u", { name: "Negado", grants: [] })
  ).rejects.toThrow("permissão");
  const admin = await service.save("a", {
    id: "admin",
    name: "Alterado",
    color: "#AA33FF",
    grants: [],
  ...
  expect(admin).toMatchObject({
    ...
    color: "#aa33ff",
    protected: true,
    grants: ["tasks:read", "tasks:create", "access:manage"],
  });
  const role = await service.save("a", {
    name: "Leitor",
    grants: ["tasks:read"],
  });
  await expect(
    service.save("a", { name: "leitor", grants: [] })
  ).rejects.toThrow("Já existe");
  await service.assign("a", "u", role.id);
  await expect(service.remove("a", role.id)).rejects.toThrow("Reatribua");
  await expect(service.assign("a", "a", "user")).rejects.toThrow(
    "administrador ativo"
  );
...
test("administrador recebe funcionalidades futuras, sem liberar outras contas", () => {
  ...
  expect(effectiveRoleGrants({ id: "admin", grants: [] }, available)).toEqual([
    ...available,
    "access:manage",
  ]);
  expect(
    effectiveRoleGrants({ id: "user", grants: ["tasks:read"] }, available)
  ).toEqual(["tasks:read"]);
  expect(
    effectiveRoleGrants(
      { id: "custom", grants: ["access:manage", "removed:action"] },
      available
    )
  ).toEqual([]);
});
