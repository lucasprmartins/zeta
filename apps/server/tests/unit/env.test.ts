...
test("rejeita configuração incompleta, segredo fraco e porta inválida", () => {
  ...
  expect(() => readEnv({ ...valid, BETTER_AUTH_SECRET: "short" })).toThrow("BETTER_AUTH_SECRET");
  expect(() => readEnv({ ...valid, PORT: "0" })).toThrow("PORT");
  expect(() => readEnv({ ...valid, DATABASE_URL: "https://example.com" })).toThrow("PostgreSQL");
});

test("aceita apenas origens explícitas para o frontend", () => {
  expect(readEnv({ ...valid, TRUSTED_ORIGINS: "http://localhost:3001, https://app.example.com" }).trustedOrigins)
  ...
  expect(() => readEnv({ ...valid, TRUSTED_ORIGINS: "https://example.com/path" })).toThrow("TRUSTED_ORIGINS");
});
